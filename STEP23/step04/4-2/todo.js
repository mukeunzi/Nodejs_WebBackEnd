...
class Todo {
	...
	setTodoCount(data) {
		return data.reduce((acc, cur) => {
			acc[cur.status] = acc[cur.status] === undefined ? [cur.name] : acc[cur.status].concat(cur.name);
			return acc;
		}, {});
	}
	...
	delete(id) {
		...
		const returnMessage = beforeLen === this.data.length ? message.NOT_EXISTED_ID : message.DELETED_DATA(name);
	...
	update(id, status) {
		...
		const returnMessage = name === undefined ? message.NOT_EXISTED_ID : message.UPDATE_DATA(name, status);
